...
// Initialize Groq client with FREE model
...
app.post('/api/financial-search', async (req, res) => {
  try {
    ...
    if (error.status === 401) {
      ...
      errorMessage = 'Too many requests to Groq API. Please try again later.';
    } else if (error.status === 404) {
      errorMessage = 'Groq API endpoint not found. Please check configuration.';
    }
